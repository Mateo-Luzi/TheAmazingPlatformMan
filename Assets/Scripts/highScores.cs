...
public class highScores : MonoBehaviour {
	...
	void Start () {
		if (PlayerPrefs.HasKey ("alpha_demo"))
			this.demoHighScore.text = PlayerPrefs.GetFloat("alpha_demo").ToString("F2");
		else
			this.demoHighScore.text = "-";

		if(PlayerPrefs.HasKey("level_001"))
			this.level1HighScore.text = PlayerPrefs.GetFloat("level_001").ToString("F2");
		else
			this.level1HighScore.text = "-";
		
		if (PlayerPrefs.HasKey ("level_002")) 
			this.level2HighScore.text = PlayerPrefs.GetFloat("level_002").ToString("F2");
		else
			this.level2HighScore.text = "-";
		
		if (PlayerPrefs.HasKey ("level_003")) 
			this.level3HighScore.text = PlayerPrefs.GetFloat ("level_003").ToString ("F2");
		else
			this.level3HighScore.text = "-";
		
		if(PlayerPrefs.HasKey("level_004"))
			this.level4HighScore.text = PlayerPrefs.GetFloat("level_004").ToString("F2");
		else
			this.level4HighScore.text = "-";
						
		if(PlayerPrefs.HasKey("level_005"))
			this.level5HighScore.text = PlayerPrefs.GetFloat("level_005").ToString("F2");
		else
			this.level5HighScore.text = "-";
							
		if(PlayerPrefs.HasKey("level_006"))
			this.level6HighScore.text = PlayerPrefs.GetFloat("level_006").ToString("F2");
		else
			this.level6HighScore.text = "-";

		if(PlayerPrefs.HasKey("level_007"))
			this.level7HighScore.text = PlayerPrefs.GetFloat("level_007").ToString("F2");
		else
			this.level7HighScore.text = "-";
									
		if(PlayerPrefs.HasKey("level_008"))
			this.level8HighScore.text = PlayerPrefs.GetFloat("level_008").ToString("F2");
		else
			this.level8HighScore.text = "-";
										
		if(PlayerPrefs.HasKey("level_009"))
			this.level9HighScore.text = PlayerPrefs.GetFloat("level_009").ToString("F2");
		else
